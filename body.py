...
class body:
  '''Metaclass; all body-classes regardless of shape should be derived from
      this one'''
  ...
  def from_dict(cls, geometry, configdict, periodicity=None):
    '''Reads body properties from doctionary and assigns them to relevant
        attributes'''
	...
	#TODO: Raise Exeption
      ...
      if spec[3] == True:
        init_args[arg + '_coordsys'] = configdict.get(arg +
            '_coordsys','lattice')
        if init_args[arg + '_coordsys'] not in ['lattice', 'cartesian']:
          exit('Error:\n' +
              'Supplied string "' + init_args[arg + '_coordsys'] + '"for ' +
              arg + '_coordsys is invalid.' + '\nExiting...\n')
