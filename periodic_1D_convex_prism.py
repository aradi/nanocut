...
class periodic_1D_convex_prism(body.body):
  ...
  def __init__(self,geometry,periodicity,planes_normal,planes_miller,shift_vector,\
               point_inside_body,order=1,shift_vector_coordsys="lattice",
               planes_normal_coordsys="lattice",\
               point_inside_body_coordsys="lattice"):
    #Initiating body using the lattice determined by geometry-object, planes given by miller indices or normal shape, a point inside the polyhedron and a possible shift_vector to determine it's distance from [0,0,0], as well as each attribute's coordinate system
    ...
    #Check if planes are parallel to axis, turn plane if not
    for plane in self._planes_normal:
      try:
        if numpy.abs(numpy.dot(plane[:3],axis.T))-NumError>0: #TODO Define NumError
    ...
    for plane in self._planes_normal:
      try:
        ...
        self._lines = numpy.vstack((self._lines,numpy.hstack((numpy.cross(
	 plane[:3],axis)[0],(numpy.dot(plane[:3],self._shift_vector.T)+plane[3])*
	 plane[:3]/numpy.linalg.norm(plane[:3])-self._shift_vector[0]))))
      except ZeroDivisionError:
        pass 
    ...
    #Distributes True and False values towards point_inside_body's respective position towards each plane
    try:
  ...
  def _from_dict_helper(cls,geometry,args,periodicity):
    return cls(geometry,periodicity,args["planes_normal"],args["planes_miller"],\
  ...
  def containing_cuboid(self,periodicity):
    ...
    #Places extreme points in cuboid by projecting axis points on plane intersection lines and returns cuboid
    corners = numpy.array([[0,0,0]])
    for line in self._lines:
      corner = numpy.dot(self._shift_vector-line[3:],line[:3].T)*line[:3]+line[3:]
      corners = numpy.vstack((corners,corner))
    
    corners = corners[1:]
    for index in range(len(corners)):
      if (numpy.isnan(corners[index]).any() or numpy.isinf(corners[index]).any() or
        numpy.isneginf(corners[index]).any()):
  ...
  def atoms_inside(self,atoms,periodicity=None):
    #Creates array assigning True and False values to points in and out of planes' boundaries respectively
    
    atoms_inside_body=numpy.zeros(atoms.shape[0],bool)
    axis=periodicity.get_axis("cartesian")
    
    #Determines for each point given if it shares the same position related to each plane as the point_inside_body and if it's projection is within the axis range
    for index in range(len(atoms)):
      TF_planes = numpy.array([(self._planes_normal[plane_idx,3]-
          sum((atoms[index,:3]-self._shift_vector)[0]*self._planes_normal[plane_idx,:3]))/
          sum(self._planes_normal[plane_idx,:3]**2) <= 0
          for plane_idx in range(len(self._planes_normal))])
      TF_axis = 0<=numpy.dot((atoms[index,:3]-self._shift_vector),axis.T)/numpy.linalg.norm(axis)**2<=1
      atoms_inside_body[index] = (TF_planes==self._parameter).all() and TF_axis
