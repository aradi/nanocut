...
class periodic_1D_convex_prism(body.body):
    ...
    def __init__(self,geometry,periodicity, planes_normal,planes_miller, shift_vector, 
        ...
        if (planes_miller==0).all():
            ...
            planes_miller = numpy.array([ numpy.hstack(( self.miller_to_normal(geometry,
        ...
        while idx1 < (self._planes_normal.shape[0]-1):
            while idx2 < self._planes_normal.shape[0]:
                if (self._planes_normal[idx1,3]==self._planes_normal[idx2,3] and (numpy.cross(
                    self._planes_normal[idx1,:3], self._planes_normal[idx2,:3]) == 0).all() and 
        ...
        for plane in self._planes_normal:
            self._lines = numpy.vstack((self._lines, numpy.hstack((
        ...
        for line in self._lines:
            corner = numpy.dot( line[3:],  line[:3].T) * line[:3] + line[3:]
