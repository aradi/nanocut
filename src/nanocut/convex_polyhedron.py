...
class convex_polyhedron(body.body):
    ...
    def __init__(self, geometry, planes_normal, planes_miller, shift_vector, order=1, 
        shift_vector_coordsys="lattice", planes_normal_coordsys="lattice"):
        ...
        planes_miller[:,0:3] = nc.miller_to_normal(geometry._lattice_vectors, planes_miller[:,0:3])
        ...
        while idx1 < self._planes_normal.shape[0] - 1:
            while idx2 < self._planes_normal.shape[0]:
                ...
                cross = np.cross(self._planes_normal[idx1,:3], self._planes_normal[idx2,:3])
    ...
    def atoms_inside(self, atoms, periodicity=None):
        '''Creates array assigning True and False values to points in and out of each
        plane's boundaries respectively'''
        # Calculates point_inside_body
        point_inside_body = (np.sum(self._corners, axis=0) / float(len(self._corners)))
        ...
        sign_atoms = (self._planes_normal[:,3,np.newaxis] - np.dot(self._planes_normal[:,:3], 
            atoms_relative)) <= 0.0
